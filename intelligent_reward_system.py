...
class IntelligentRewardSystem:
    ...
    def _calculate_positional_reward(self, old_state, move, new_state) -> float:
        ...
        if escape_zone:
            ...
            if is_escaping:
                # 移動した駒が善玉かどうか判定
                player_pieces = (old_state.player_a_pieces if current_player == "A" 
                               else old_state.player_b_pieces)
    ...
    def _calculate_tactical_reward(self, old_state, move, new_state) -> float:
        ...
        current_pieces = (old_state.player_a_pieces if current_player == "A" 
                         else old_state.player_b_pieces)
    ...
    def _calculate_strategic_reward(self, old_state, move, new_state) -> float:
        ...
        # 相手機動性削減
        opponent_mobility = self._calculate_mobility(new_state, 
                                                   "B" if current_player == "A" else "A")
    ...
    def _calculate_danger_level(self, position, game_state, current_player) -> float:
        ...
        # 相手駒との距離分析
        opponent_pieces = (game_state.player_b_pieces if current_player == "A" 
    ...
    def _calculate_protection_bonus(self, position, game_state, current_player) -> float:
        ...
        # 味方駒との距離分析
        friendly_pieces = (game_state.player_a_pieces if current_player == "A" 
                          else game_state.player_b_pieces)
    ...
    def _evaluate_formation(self, game_state, current_player) -> float:
        """フォーメーション評価"""
        pieces = (game_state.player_a_pieces if current_player == "A" 
                 else game_state.player_b_pieces)
    ...
    def _calculate_mobility(self, game_state, player) -> float:
        """機動性計算"""
        pieces = (game_state.player_a_pieces if player == "A" 
                 else game_state.player_b_pieces)
