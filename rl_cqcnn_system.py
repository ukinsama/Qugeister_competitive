...
class RLRewardSystem:
    ...
    def calculate_step_reward(self, old_state: Dict, action: Tuple, 
                             new_state: Dict, estimation: Dict) -> float:
        """1ステップの報酬を計算"""
        reward = 0.0
    ...
    def calculate_episode_reward(self, winner: str, player: str, 
                                final_estimations: List[Dict]) -> float:
        """エピソード終了時の報酬を計算"""
        reward = 0.0
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'avg_estimation_accuracy': np.mean(self.estimation_accuracy_history[-100:]) if self.estimation_accuracy_history else 0,
...
class RLCQCNNTrainer:
    ...
    def __init__(self, estimator: CQCNNPieceEstimator, 
        ...
        self.target_estimator = CQCNNPieceEstimator(
            estimator.n_qubits, 
            estimator.n_layers
        )
    ...
    def select_action(self, estimations: Dict, legal_moves: List, 
                     q_map: np.ndarray) -> Tuple:
        """ε-greedy行動選択"""
            ...
            for pos in experience.enemy_positions:
                ...
                for layer in range(self.estimator.n_layers):
                    ...
                    for q in range(n_qubits):
                        ...
                        # インプレース操作を避ける
                        new_q_state[:, q] = (
                            torch.sin(rotation[0] * quantum_input[:, q % quantum_input.shape[1]]) * 
                            torch.cos(rotation[1] * quantum_input[:, (q+1) % quantum_input.shape[1]]) + 
...
class SelfPlaySystem:
    ...
    def play_episode(self, verbose: bool = False) -> Dict:
        ...
        for turn in range(50):
            ...
            enemy_positions = [(i, j) for i in range(6) for j in range(6) 
                             if board[j, i] == -1][:4]
...
class ComparisonExperiment:
    ...
    def train_rl_model(self, episodes: int = 100, verbose: bool = True):
        ...
        for episode in range(episodes):
            ...
            # 学習実行
            train_result = self.rl_trainer.train_episode(
                lambda trainer, verbose: result,
                verbose=False
            )
                ...
                with torch.no_grad():
                    ...
                    for layer in range(self.sl_system.estimator.n_layers):
                        ...
                        for q in range(n_qubits_sl):
                            ...
                            new_q_state_sl[:, q] = (
                                torch.sin(rotation_sl[0] * quantum_input_sl[:, q % quantum_input_sl.shape[1]]) * 
                                torch.cos(rotation_sl[1] * quantum_input_sl[:, (q+1) % quantum_input_sl.shape[1]]) + 
                    ...
                    for layer in range(self.rl_estimator.n_layers):
                        ...
                        for q in range(n_qubits_rl):
                            ...
                            new_q_state_rl[:, q] = (
                                torch.sin(rotation_rl[0] * quantum_input_rl[:, q % quantum_input_rl.shape[1]]) * 
                                torch.cos(rotation_rl[1] * quantum_input_rl[:, (q+1) % quantum_input_rl.shape[1]]) + 
    ...
    def plot_comparison(self):
        ...
        # 報酬推移
        ax1.plot(self.rl_trainer.training_history['episodes'],
                self.rl_trainer.training_history['rewards'], 'b-', alpha=0.7)
        ...
        # 損失推移
        ax2.plot(self.rl_trainer.training_history['episodes'],
                self.rl_trainer.training_history['losses'], 'r-', alpha=0.7)
