...
class DataProcessor:
    ...
    @staticmethod
    def prepare_7channel_tensor(board: np.ndarray, player: str, 
...
class CQCNNModel(nn.Module):
    ...
    def __init__(self, n_qubits: int = 6, n_layers: int = 3):
        ...
        self.quantum_layers = nn.ModuleList([
            nn.Linear(self.quantum_dim, self.quantum_dim) for _ in range(n_layers)
        ])
...
class PieceEstimator(ABC):
    ...
    def estimate(self, board: np.ndarray, enemy_positions: List[Tuple[int, int]], 
                player: str, my_pieces: Dict[Tuple[int, int], str], 
...
class CQCNNEstimator(PieceEstimator):
    ...
    def estimate(self, board: np.ndarray, enemy_positions: List[Tuple[int, int]], 
                player: str, my_pieces: Dict, turn: int) -> Dict:
        """敵駒推定"""
...
class RewardFunction(ABC):
    ...
    def calculate_move_reward(self, game_state: GameState, move: Tuple, 
                            player: str, my_pieces: Dict) -> float:
        """手の報酬を計算"""
        pass
