...
# TODO: Import these from the actual game module when available
# from game_module import GeisterGame, RandomAI
...
class RealGameDataGenerator:
    ...
    def generate_game_data(num_games: int = 100) -> List[Dict]:
        ...
        for game_idx in range(num_games):
            # TODO: Uncomment when GeisterGame and RandomAI are imported
            # game = GeisterGame()
            # player_a = RandomAI("A")
            # player_b = RandomAI("B")
    ...
    def prepare_batch(data_list: List[Dict], batch_size: int = 32):
        ...
        for data in batch_data:
            # ボードをテンソルに変換
            tensor = RealGameDataGenerator.board_to_tensor(data["board"], data["player"])
...
class ImprovedTrainer:
    ...
    def __init__(self, model, learning_rate=0.001):
        ...
        self.history = {"loss": [], "accuracy": [], "val_accuracy": []}

    def train_with_real_data(
        self, train_data: List[Dict], val_data: Optional[List[Dict]] = None, epochs: int = 50, batch_size: int = 32
    ):
        """実データで学習"""
        print("\n🎓 実データで学習開始")
        ...
        for epoch in range(epochs):
            ...
            # 進捗表示
            if (epoch + 1) % 10 == 0:
                print(f"Epoch {epoch + 1:3d}/{epochs} | Loss: {avg_loss:.4f} | Train Acc: {train_accuracy:.3f}", end="")
                if val_data:
...
def main():
    ...
    trainer.train_with_real_data(train_data, val_data, epochs=50, batch_size=32)
