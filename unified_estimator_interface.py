...
class PieceEstimatorInterface(ABC):
    ...
    @abstractmethod
    def estimate(
        self, board: np.ndarray, enemy_positions: List[Tuple[int, int]], player_id: str, return_confidence: bool = False
    ) -> Union[Dict, EstimationResult]:
        """
        敵駒のタイプを推定
    ...
    def train(
        ...
        validation_data: Optional[List[TrainingData]] = None,
    ) -> Dict[str, float]:
        """
        モデルを学習
...
class BaseCQCNNEstimator(PieceEstimatorInterface):
    ...
    def _prepare_input(self, board: np.ndarray, position: Tuple[int, int]) -> torch.Tensor:
        """
        入力データを準備
...
def example_usage():
    ...
    # 設定作成
    config = EstimatorConfig(n_qubits=8, n_layers=3, learning_rate=0.001, batch_size=32)
