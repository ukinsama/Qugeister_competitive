...
class EstimatorModule(ABC):
    ...
    @abstractmethod
    def estimate(
        self, board: np.ndarray, enemy_positions: List[Tuple[int, int]], player: str
        ...
        class CQCNNModel(nn.Module):
            def __init__(self, n_qubits, n_layers):
                ...
                self.reduction = nn.Sequential(
                    nn.Linear(64 * 3 * 3, 64), nn.ReLU(), nn.Dropout(0.2), nn.Linear(64, n_qubits), nn.Tanh()
                )
                ...
                # 出力層
                self.output = nn.Sequential(nn.Linear(n_qubits, 32), nn.ReLU(), nn.Linear(32, 2))
...
class RandomEstimator(EstimatorModule):
    ...
    def estimate(
        ...
        for pos in enemy_positions:
            ...
            results[pos] = {"good_prob": good_prob, "bad_prob": bad_prob, "confidence": max(good_prob, bad_prob)}
...
class QMapGenerator(ABC):
    ...
    @abstractmethod
    def generate(self, board: np.ndarray, estimations: Dict, my_pieces: Dict, player: str) -> np.ndarray:
        """Q値マップを生成"""
        pass
...
class ActionSelector(ABC):
    ...
    @abstractmethod
    def select(self, q_map: np.ndarray, legal_moves: List[Tuple], temperature: float = 1.0) -> Tuple:
        """行動を選択"""
        pass
...
class ModularAgent:
    ...
    def _generate_name(self) -> str:
        return (
            f"Agent_{self.player_id}[{self.config.placement.get_name()[:3]}+"
    ...
    def get_move(self, game_state, legal_moves: List[Tuple]) -> Tuple:
        ...
        try:
            ...
            estimations = {}
            if enemy_positions:
                estimations = self.config.estimator.estimate(game_state.board, enemy_positions, self.player_id)

            # 自分の駒情報
            my_pieces = game_state.player_a_pieces if self.player_id == "A" else game_state.player_b_pieces

            # Q値マップを生成
            q_map = self.config.qmap_generator.generate(game_state.board, estimations, my_pieces, self.player_id)
...
class CompetitionRunner:
    ...
    def __init__(self):
        # 利用可能なモジュール
        self.modules = {
            "placement": [StandardPlacement(), DefensivePlacement(), AggressivePlacement(), RandomPlacement()],
            "estimator": [
    ...
    def create_agent(
        self, player_id: str, placement_idx: int = 0, estimator_idx: int = 0, qmap_idx: int = 0, selector_idx: int = 0
    ) -> ModularAgent:
    ...
    def run_tournament(self, n_agents: int = 4):
        ...
        sorted_results = sorted(results.items(), key=lambda x: (x[1]["wins"], -x[1]["losses"]), reverse=True)
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            runner.run_match(agent1, agent2, verbose=True)
