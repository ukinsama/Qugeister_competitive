...
class CQCNNDataCollector:
    ...
    def _calculate_confidence_hint(self, snapshot: BoardSnapshot, 
                                  pos: Tuple[int, int], 
                                  player_view: str) -> float:
        """確信度のヒントを計算（位置や状況から）"""
        x, y = pos
...
class DataCollectionGame:
    ...
    def run_game_with_collection(self, agent1: Any, agent2: Any, 
...
def run_data_collection_campaign(num_games: int = 10):
    ...
    for game_num in range(num_games):
        ...
        # データ収集しながらゲーム実行
        result = game_runner.run_game_with_collection(agent1, agent2, verbose=True)
