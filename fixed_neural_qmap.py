...
class FixedNeuralQMapGenerator:
    ...
    def generate(
        ...
        if not hasattr(self, "_debug_shown"):
            print(
                f"  [NeuralQMap] 入力次元: {len(all_features)} (ボード:{len(board_features)} + 推定:{len(est_features)})"
            )
    ...
    def _extract_estimation_features(self, estimations: Dict, my_pieces: Dict) -> np.ndarray:
        ...
        if estimations:
            ...
            [e["bad_prob"] for e in estimations.values()]
    ...
    def _postprocess_qmap(self, q_map: np.ndarray, my_pieces: Dict, player: str, estimations: Dict) -> np.ndarray:
        """Q値マップの後処理"""
        # スケーリング
        q_map = q_map * 5.0
...
def test_fixed_neural_qmap():
    ...
    my_pieces = {(1, 1): "good", (2, 1): "bad", (3, 2): "good"}
