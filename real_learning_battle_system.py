...
class CQCNNModel(nn.Module):
    ...
    def quantum_layer(self, x):
        ...
        # "量子的"な非線形変換
        x = self.quantum_linear(x)  # -> self.quantum_feature_dim (32次元)
        x = torch.tanh(x)  # 量子状態の範囲を模擬
        
        # 量子干渉効果を模擬（実数部と虚数部）
        x_real = x[:, :self.quantum_feature_dim//2]  # 16次元
        x_imag = x[:, self.quantum_feature_dim//2:]  # 16次元
        amplitude = torch.sqrt(x_real**2 + x_imag**2 + 1e-8)  # 16次元
        
        # 位相情報も追加して32次元に戻す
        phase = torch.atan2(x_imag, x_real + 1e-8)  # 16次元
        quantum_features = torch.cat([amplitude, phase], dim=1)  # 32次元
    ...
    def forward(self, x):
        ...
        # print(f"After quantum: {quantum_out.shape}")
...
class CQCNNEstimator:
    ...
    def train_supervised(self, training_data: List[Dict], config: LearningConfig) -> Dict:
        ...
        for epoch in range(config.supervised_epochs):
            ...
            print(f"📊 結果: Train Loss={train_loss:.4f} | Train Acc={train_acc:.1f}% | "
    ...
    def estimate(self, board: np.ndarray, my_pieces: Dict, enemy_positions: List[Tuple], player: str, turn: int = 1) -> Dict:
        """敵駒推定"""
...
class GameEngine:
    ...
    def play_game(self, ai1_config: ModuleConfig, ai1_estimator: CQCNNEstimator,
                  ai2_config: ModuleConfig, ai2_estimator: CQCNNEstimator) -> str:
        """ゲーム実行（簡略化）"""
